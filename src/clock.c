...
double core_mhz(int verbose) {
  /*
	...
	fprintf(stderr, "Can't open /proc/cpuinfo to get clock information;\n"
                        "CPU frequency assumed to be 1.8GHz\n");
	cpu_ghz = 1.8;
    ...
    fclose(fp);
    if (cpu_ghz == 0.0) {
	fprintf(stderr, "Can't read /proc/cpuinfo to get clock information;\n"
                        "CPU frequency assumed to be 1.8GHz\n");
	cpu_ghz = 1.8;
	return cpu_ghz * 1000.0;
    }
  ...
  */
    cpu_ghz = 1.8; // for Raspberry Pi 4
    return cpu_ghz * 1000;
}
